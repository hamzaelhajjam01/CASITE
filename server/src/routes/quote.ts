...
import type { PackageDoc, PricingRuleDoc, QuoteResponse } from '../types/index.js';
...
router.post('/quote/calculate', validate(quoteSchema), async (req, res) => {
  ...
  // 1. Resolve package
  const pkg = await db.collection<PackageDoc>('packages').findOne({ slug: package_slug, is_active: true });
  if (!pkg) { res.status(404).json({ error: `Package "${package_slug}" not found or inactive` }); return; }

  // 2. Base price from embedded prices map (stored in dollars)
  const basePrice = pkg.prices[term];
  if (basePrice === undefined) { res.status(404).json({ error: `No price for ${package_slug}/${term}` }); return; }
