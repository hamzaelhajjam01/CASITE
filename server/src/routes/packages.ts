...
import type { PackageDoc, CoverageRowDoc, PackageResponse, CoverageRowResponse } from '../types/index.js';

const router = Router();

// ─── Helper ───────────────────────────────────────────────────────

function toPackageResponse(doc: PackageDoc, coverageRows?: CoverageRowDoc[]): PackageResponse {
  ...
  if (coverageRows) {
    response.coverage_rows = coverageRows.map((r): CoverageRowResponse => ({
      id:          r._id!.toString(),
...
router.post('/admin/packages', requireAdmin, validate(createPackageSchema), async (req, res) => {
  ...
  const doc: PackageDoc = {
    ...
    is_active:   true,
    prices:      body.prices as Record<string, number>,
    created_at:  now,
    updated_at:  now,
  };
...
router.put('/admin/packages/:id', requireAdmin, validate(updatePackageSchema), async (req, res) => {
  ...
  const body = req.body as z.infer<typeof updatePackageSchema>;
  ...
  if (body.prices) {
    for (const [term, price] of Object.entries(body.prices)) {
      if (price !== undefined) $set[`prices.${term}`] = price;
    }
  }
