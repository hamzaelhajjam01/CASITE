...
import type { PricingRuleDoc, PricingRuleResponse } from '../types/index.js';

const router = Router();

// ─── Helper ───────────────────────────────────────────────────────

function toResponse(doc: PricingRuleDoc): PricingRuleResponse {
  return {
    id:          doc._id!.toString(),
...
router.get('/pricing-rules', requireAdmin, async (_req, res) => {
  ...
  const rules = await db
    .collection<PricingRuleDoc>('pricing_rules')
    .find()
...
router.post('/pricing-rules', requireAdmin, validate(createRuleSchema), async (req, res) => {
  ...
  const now = new Date();
  const doc: PricingRuleDoc = { ...body, created_at: now, updated_at: now };

  const result = await db.collection<PricingRuleDoc>('pricing_rules').insertOne(doc);
...
router.put('/pricing-rules/:id', requireAdmin, validate(updateRuleSchema), async (req, res) => {
  ...
  if (body.context     !== undefined) $set['context']     = body.context;
  if (body.match_value !== undefined) $set['match_value'] = body.match_value;
  if (body.multiplier  !== undefined) $set['multiplier']  = body.multiplier;
  ...
  if (!result) { res.status(404).json({ error: 'Pricing rule not found' }); return; }
...
router.delete('/pricing-rules/:id', requireAdmin, async (req, res) => {
  ...
  const result = await db.collection<PricingRuleDoc>('pricing_rules').findOneAndDelete({ _id: id });
  if (!result) { res.status(404).json({ error: 'Pricing rule not found' }); return; }
