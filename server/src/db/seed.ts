...
import type { PackageDoc, PricingRuleDoc, CoverageRowDoc } from '../types/index.js';
...
for (const pkg of packages) {
  ...
  await db.collection<PackageDoc>('packages').updateOne(
    { slug: pkg.slug },
    { $set: rest, $setOnInsert: { created_at } },
    { upsert: true }
  );
}
...
for (const rule of rules) {
  ...
  await db.collection<PricingRuleDoc>('pricing_rules').updateOne(
...
// ─── Coverage Rows ────────────────────────────────────────────────
const coverageRows: CoverageRowDoc[] = [
...
for (const row of coverageRows) {
  ...
  await db.collection<CoverageRowDoc>('coverage_rows').updateOne(
