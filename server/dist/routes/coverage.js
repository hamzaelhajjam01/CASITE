...
router.post('/coverage-rows', requireAdmin, validate(createRowSchema), async (req, res) => {
    ...
    const result = await db.collection('coverage_rows').insertOne(doc);
...
router.put('/coverage-rows/:id', requireAdmin, validate(updateRowSchema), async (req, res) => {
    ...
    if (body.basic_value !== undefined)
        $set['basic_value'] = body.basic_value;
    if (body.full_value !== undefined)
        $set['full_value'] = body.full_value;
    ...
    if (!result) {
        res.status(404).json({ error: 'Coverage row not found' });
        return;
    }
